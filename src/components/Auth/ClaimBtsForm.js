...
const renderField = ({ tabIndex, className, errors, placeholder, input, label, type, iconClass, meta: { touched, error, dirty} }) => (
    ...
    <label className="row">
        <Translate component="input" autoFocus={tabIndex === '1'} autoComplete="off" {...input} type={type} placeholder={placeholder} tabIndex={tabIndex} className={(dirty && touched && error) ? (className + ' error') : className} attributes={{ placeholder: placeholder }} />

        <span className="fieldPic">
                <span className={iconClass}/>
            </span>
        {touched && dirty && error && <span className="error__hint">{error}</span>}
...
class ClaimBtsForm extends React.Component {
    ...
    render() {
        ...
        switch(btnStatus) {
            case 'default':
                SubmitButton = (
                    <button className="btn btn-sbm btn-fsz-18 pull-right" type="submit" disabled={invalid || submitting}><span className="btnText">{counterpart.translate("login_bts.login_btn")}</span></button>
                );
                break;
            case 'loading':
        ...
        return (
            <form onSubmit={handleSubmit}>
                ...
                <Field name="private_bts_key" errors={errors} className="field field-pic" iconClass="fieldIcon icon-password" component={renderField} placeholder="login_bts.login_key_placeholder"  type="password" tabIndex="1" />
    ...
    validate: (values) => {
        ...
        if (!values.private_bts_key) {
            ...
            try {
                ...
                errors.private_bts_key = counterpart.translate("errors.paste_your_redemption_key_here", {cnt: MAX_PASSWORD_CHARACTERS});//e.message;
            }
        }
