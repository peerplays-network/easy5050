    ...
    return (
        ...
        <td className="td" id="tableCustom">
            <span className="td__cell" id="strongText">
                ...
                type === 79 ? ' - ' + Helper.currencyConvert(new BigNumber(amount.amount || 0).dividedBy(Math.pow(10, 9)).toNumber().toFixed(9)) :
                type === 5 ? '-' :
                ...
                type === 78 ? ' - ' + tickets_to_buy : '-'}  {
                    ...
                    '':
                    type === 78 ?
                    ' @ ' + Helper.currencyConvert(new BigNumber(assetPrice || 0).dividedBy(Math.pow(10, 9)).toNumber().toFixed(9)) + '':
                    type === 5 ? 0 :
                    ''
                } 
            </span>
        ...
        <td className="td" id="tableCustom">
            ...
            {type === 78 ? Helper.currencyConvert((new BigNumber(assetPrice || 0).dividedBy(Math.pow(10, 9)).toNumber().toFixed(9)) * (tickets_to_buy)) : 
            type === 79 ? Helper.currencyConvert(new BigNumber(amount.amount || 0).dividedBy(Math.pow(10, 9)).toNumber().toFixed(9)) :
            type === 0 ? Helper.currencyConvert(new BigNumber(amount.amount || 0).dividedBy(Math.pow(10, 9)).toNumber().toFixed(9)) :
            type === 10 ? '-' :
             '-'}
            </span>
        </td>
